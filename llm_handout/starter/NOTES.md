...
everything else), WSD (warmup-stable-decay) learning-rate schedule, batch 64, 2000 steps.
Dev bpb: 2.3718 (baseline) → **1.6777** (final, -29.3% relative).
...
and WSD each won on their own, QK-Norm was a wash. We then stacked the three winners
(RoPE + Muon + WSD), which beat every individual run cleanly with no interference, and
finally pushed batch size to 64 on top of that stack for another clear win (~4.8 epochs of
the corpus, still within Muennighoff's safe range for repeated-data training). See
